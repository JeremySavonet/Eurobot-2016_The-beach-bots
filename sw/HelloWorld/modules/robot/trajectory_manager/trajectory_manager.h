 ...
 *
 * This module only works with a control system made with
...
enum trajectory_state
{
    ...
    // clitoid
    RUNNING_CLITOID_LINE,  // Running a clitoid ( line->circle->line ) in the
                           // line part.
...
{
    enum trajectory_state
        state; // describe the type of target, and if we reached the target
    ...
    double a_start_rad; // in xy consigns, start to move in distance when
                        // a_target < a_start
    ...
    double cs_hz; // The frequency of the control system associated with this
                  // manager.
 ...
 *
 * Sets the robot_pos and robot_system used for the computation of the
 * trajectory.
