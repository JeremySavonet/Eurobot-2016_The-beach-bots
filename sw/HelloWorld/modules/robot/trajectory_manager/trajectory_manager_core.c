 ...
 */
static void __trajectory_goto_d_a_rel( struct trajectory* traj,
                                       double d_mm,
                                       double a_rad,
                                       uint8_t state,
                                       uint8_t flags )
{
    ...
    if( flags & UPDATE_A )
    {
        ...
        {
            a_consign = ( int32_t )(
                a_rad * ( traj->position->phys.distance_imp_per_mm ) *
    ...
    if( flags & UPDATE_D )
    {
        ...
        {
            d_consign = ( int32_t )(
                ( d_mm ) * ( traj->position->phys.distance_imp_per_mm ) );
        }
...
{
    __trajectory_goto_d_a_rel( traj,
                               d_mm,
                               0,
                               RUNNING_D,
...
{
    __trajectory_goto_d_a_rel( traj, 0, RAD( a_deg ), RUNNING_A, UPDATE_A );
}
...
void trajectory_goto_backward_xy_abs( struct trajectory* traj,
                                      double x,
                                      double y )
{
...
{
    return trajectory_distance_finished( traj ) &&
...
void trajectory_manager_xy_event( struct trajectory* traj )
{
    ...
    case RUNNING_XY_B_ANGLE_OK:
         ...
         */
        if( ABS( v2pol_target.theta ) >
            traj->a_start_rad ) // || ABS(v2pol_target.r) < traj->d_win)
            set_quadramp_speed( traj, 0, traj->a_speed );
        else
        {
            coef = ( traj->a_start_rad - ABS( v2pol_target.theta ) ) /
                   traj->a_start_rad;
            set_quadramp_speed( traj, traj->d_speed * coef, traj->a_speed );
        }

        d_consign = ( int32_t )( v2pol_target.r *
    ...
    // EVT_DEBUG(E_TRAJECTORY,"EVENT XY d_cur=%" PRIi32 ", d_consign=%" PRIi32
    // ", d_speed=%" PRIi32 ", "
...
void trajectory_manager_circle_event( struct trajectory* traj )
{
    ...
    circle_get_da_speed_from_radius( traj,
                                     radius / ( coef_p * coef_d ),
                                     & d_speed,
                                     & a_speed );

    set_quadramp_speed( traj, d_speed, a_speed );

    // EVT_DEBUG(E_TRAJECTORY, "angle=%2.2f radius=%2.2f r=%2.2f coef_p=%2.2f
    // coef_d=%2.2f "
                            ...
                            uint8_t flags )
{
    ...
    dst_distance = RAD( rel_a_deg ) *
                   ( traj->position->phys.distance_imp_per_mm ) * radius_mm;
    ...
    // DEBUG(E_TRAJECTORY, "Circle rel (x,y)=%2.2f,%2.2f r=%2.2f flags=%x
    // dst_angle=%"PRIi32"",
 ...
 */
double circle_get_dist_from_degrees( double radius_mm, double a_deg )
{
                            ...
                            double* remain_d_mm_out )
{
    ...
    // DEBUG(E_TRAJECTORY, "R_mm=%2.2f a_rad=%2.2f alpha_rad=%2.2f
    // beta_rad=%2.2f t=%2.2f",
                           ...
                           double d_inter_mm )
{
    ...
    __trajectory_line_abs( traj, x, y, turnx, turny, advance );
