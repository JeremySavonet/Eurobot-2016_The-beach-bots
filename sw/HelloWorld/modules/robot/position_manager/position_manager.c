...
void position_manage( struct robot_position* pos )
{
    ...
    if( 0 == delta.angle )
    {
        // we go straight
        dx = cos( a ) *
             ( ( double )delta.distance / ( pos->phys.distance_imp_per_mm ) );
        dy = sin( a ) *
             ( ( double )delta.distance / ( pos->phys.distance_imp_per_mm ) );
        x += dx;
        y += dy;
    ...
    else
    {
        ...
        r = ( double )delta.distance * pos->phys.track_mm /
            ( ( double )delta.angle * 2 );
        arc_angle = 2 * ( double )delta.angle /
                    ( pos->phys.track_mm * pos->phys.distance_imp_per_mm );
