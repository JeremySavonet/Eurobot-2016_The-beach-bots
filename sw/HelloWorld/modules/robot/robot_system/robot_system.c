 ...
 */
static inline void safe_setpwm( void ( *f )( void*, int32_t ),
                                void* param,
                                int32_t value )
{
...
void rs_update( void* data )
{
    ...
    // read encoders
    wext.left =
        safe_getencoder( rs->left_ext_encoder, rs->left_ext_encoder_param );
    wext.right =
        safe_getencoder( rs->right_ext_encoder, rs->right_ext_encoder_param );

#ifdef CONFIG_MODULE_ROBOT_SYSTEM_MOT_AND_EXT
    wmot.left =
        safe_getencoder( rs->left_mot_encoder, rs->left_mot_encoder_param );
    wmot.right =
        safe_getencoder( rs->right_mot_encoder, rs->right_mot_encoder_param );
#endif
