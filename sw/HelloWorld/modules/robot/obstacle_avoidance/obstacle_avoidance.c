...
static void __oa_start_end_points( int32_t st_x,
                                   int32_t st_y,
                                   int32_t en_x,
                                   int32_t en_y );
...
void oa_start_end_points( int32_t st_x,
                          ...
                          int32_t en_y )
{
    DEBUG_OA_PRINTF( "%s() (%ld,%ld) (%ld,%ld)\r",
                     __FUNCTION__,
                     st_x,
                     st_y,
                     en_x,
...
void dijkstra( uint8_t start_p, uint8_t start )
{
    ...
    while( ! finish )
    {
        ...
        for( start_p = 0; start_p < MAX_POLY; start_p++ )
        {
            ...
            {
                if( oa.valid[ GET_PT( oa.polys[ start_p ].pts[ start ] ) ] !=
                    2 )
                    continue;
                add = -2;
             ...
             * we wtep 2 by 2. */
                for( i = 0; i < oa.ray_n; i += 2 )
                {
                    ...
                    if( start_p != oa.u.rays[ i ] ||
                        start != oa.u.rays[ i + 1 ] )
                        continue;

                    if( ( oa.pweight[ GET_PT(
                              oa.polys[ oa.u.rays[ i + add ] ]
                                  .pts[ oa.u.rays[ i + add + 1 ] ] ) ] != 0 ) &&
                        ( oa.pweight[ GET_PT( oa.polys[ start_p ]
                                                  .pts[ start ] ) ] +
                              oa.weight[ i / 4 ] >=
                          oa.pweight[ GET_PT(
                              oa.polys[ oa.u.rays[ i + add ] ]
                                  .pts[ oa.u.rays[ i + add + 1 ] ] ) ] ) )
                        continue;

                    oa.p[ GET_PT( oa.polys[ oa.u.rays[ i + add ] ]
                                      .pts[ oa.u.rays[ i + add + 1 ] ] ) ] =
                        start_p;
                    oa.pt[ GET_PT( oa.polys[ oa.u.rays[ i + add ] ]
                                       .pts[ oa.u.rays[ i + add + 1 ] ] ) ] =
                        start;
                    oa.valid[ GET_PT( oa.polys[ oa.u.rays[ i + add ] ]
                                          .pts[ oa.u.rays[ i + add + 1 ] ] ) ] =
                        2;
                    oa.pweight[ GET_PT(
                        oa.polys[ oa.u.rays[ i + add ] ]
                    ...
                    finish = 0;
                    DEBUG_OA_PRINTF(
                        "%s() (%2.0f,%2.0f p=%ld) %d (%2.0f,%2.0f p=%ld)\r",
                        __FUNCTION__,
...
int8_t get_path( poly_t* polys )
{
    ...
    while( ! ( p == 0 && pt == 0 ) )
    {
        ...
        DEBUG_OA_PRINTF( "result[%d]: %2.0f, %2.0f\r",
                         i,
                         oa.u.res[ i ].x,
                         oa.u.res[ i ].y );
        i++;
