...
#if DEBUG_PL == 1
#define DEBUG_POLY_PRINTF( lvl, args... ) \
    DPRINT( lvl, KYEL args, ##__VA_ARGS__ )
#else
...
{
    if( p->x >= bbox_x1 && p->x <= bbox_x2 && p->y >= bbox_y1 &&
        p->y <= bbox_y2 )
    {
        return 1;
    }
                          ...
                          poly_t* pol )
{
    ...
    DEBUG_POLY_PRINTF( 3,
                       "is in poly: p1 %d p2: %d cpt %d\r\n",
                       ret1,
                       ret2,
                       cpt );

    DEBUG_POLY_PRINTF( 3,
                       "p intersect: %" PRIi32 " %" PRIi32 "\r\n",
                       p.x,
                       p.y );
    ...
    for( i = 0; i < npolys; i++ )
    {
        ...
        for( ii = 0; ii < polys[ i ].l; ii++ )
        {
            ...
            for( index = 1; index < npolys; index++ )
            {
                ...
                if( is_crossing_poly( polys[ i ].pts[ ii ],
                                      polys[ i ].pts[ n ],
                                      NULL,
                                      & polys[ index ] ) == 1 )
                {
                       ...
                       uint16_t* weight )
{
    ...
    for( i = 0; i < ray_n; i += 4 )
    {
        v.x = polys[ rays[ i ] ].pts[ rays[ i + 1 ] ].x -
              polys[ rays[ i + 2 ] ].pts[ rays[ i + 3 ] ].x;
        v.y = polys[ rays[ i ] ].pts[ rays[ i + 1 ] ].y -
              polys[ rays[ i + 2 ] ].pts[ rays[ i + 3 ] ].y;
