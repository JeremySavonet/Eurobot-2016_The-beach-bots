    ...
    if( dir == _HIST_UP )
    {
        if( cnt >= this->cur )
        {
            ...
            if( this->ring_buf[ header ] )
            {
                ...
                {
                    memcpy( line,
                            this->ring_buf + header + 1,
                            this->ring_buf[ header ] );
                }
                else
                {
...
static void terminal_reset_cursor( microrl_t* this )
{
    ...
    snprintf( str,
              16,
              "\033[%dD\033[%dC",
              _COMMAND_LINE_LEN + _PROMPT_LEN + 2,
              _PROMPT_LEN );
...
//*****************************************************************************
void microrl_set_complete_callback( microrl_t* this,
...
//*****************************************************************************
void microrl_set_execute_callback( microrl_t* this,
...
#ifdef _USE_CTLR_C
//*****************************************************************************
void microrl_set_sigint_callback( microrl_t* this,
                                  void ( *sigintf )( struct microrl_t* ) )
{
    this->sigint = sigintf;
}
#endif
...
static void microrl_get_complite( microrl_t* this )
{
    ...
    if( compl_token[ 0 ] != NULL )
    {
        ...
        if( len )
        {
            microrl_insert_text( this,
                                 compl_token[ 0 ] +
                                     strlen( this->tkn_arr[ status - 1 ] ),
