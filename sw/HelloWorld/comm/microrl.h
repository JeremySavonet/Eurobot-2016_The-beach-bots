...
struct microrl_t
{
    ...
    char* tkn_arr[ _COMMAND_TOKEN_NMB ]; // array of token for call 'execute'
                                         // callback
    int ( *execute )( int argc, char** argv ); // ptr to 'execute' callback
    char** ( *get_completion )( int argc,
                                char** argv ); // ptr to 'completion' callback
...
//   Empty string if complite not found, and multiple string if there are some
//   token
void microrl_set_complete_callback( microrl_t* _this,
                                    char** ( *get_completion )( int, char** ) );
...
// string
void microrl_set_execute_callback( microrl_t* _this,
                                   int ( *execute )( int, char** ) );

// set callback for Ctrl+C terminal signal
#ifdef _USE_CTLR_C
void microrl_set_sigint_callback( microrl_t* _this,
                                  void ( *sigintf )( struct microrl_t* this ) );
#endif
